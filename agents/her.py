...
from agents.helpers import HindsightExperienceReplayWrapper, KEY_TO_GOAL_STRATEGY
from stable_baselines.her.utils import HERGoalEnvWrapper

from agents.base_class import BaseRLModel, OffPolicyRLModel
...
class HER(BaseRLModel):
    ...
    def _save_to_file(self, save_path, data=None, params=None, cloudpickle=False):
        ...
        print(
            f"\nWarning: the model class (TD3, DDPG, ...) used with HER cannot be saved currently due to a serialization issue.")
    ...
    @classmethod
    def load(cls, load_path, model_class, env=None, custom_objects=None, **kwargs):
        ...
        model = cls(policy=data["policy"], env=env, model_class=model_class,
        ...
        print(
            f"\nWarning: the model class (TD3, DDPG, ...) used with HER cannot be saved currently due to a serialization issue --> make sure you use the right one when loading!")
        model.model = model_class.load(load_path, model.get_env(), **kwargs)
