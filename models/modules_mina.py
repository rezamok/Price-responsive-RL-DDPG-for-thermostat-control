...
class PositiveLinear(nn.Module):
    ...
    def forward(self, input):
        if self.require_bias:
            ...
            # print("ghghghg")
            # print(type(input))
            # print(self.log_weight.exp())
            input = input.double()
            b = self.log_weight.exp().double()
            a = nn.functional.linear(input, b)
...
class PCNN(nn.Module):
    ...
    def _build_model(self) -> None:
        ...
        if self.feed_input_through_nn:
            # print("aaaab")
            # print(len(self.inputs_D))
            # print(self.input_nn_hidden_sizes)
    ...
    def forward(self, x_: torch.Tensor, states=None, warm_start: bool = False) -> dict:  # noqa: C901
        ...
        if not warm_start:
            ...
            x[:, -1, self.temperature_column] = x[:, -1, self.temperature_column].clone() - self.last_E.item()
        ...
        if self.feed_input_through_nn:
            ...
            for time_step in range(x.shape[1]):
                ...
                # print(temp[0])
                #print(self.input_nn)
                for sequential_block in self.input_nn:
                    for layer in sequential_block:
                        if isinstance(layer, nn.Linear):
                            ...
                            if layer.weight.dtype == torch.float64:
                                ...
                                layer.weight = nn.Parameter(layer.weight.float())
                                # temp = temp.to(torch.float64)
                for layer in self.input_nn:
                    temp = layer(temp[0])
                    if temp.dtype == torch.float64:
                        temp = temp.to(torch.float32)
        ...
        D_embedding = D_embedding.to(default_dtype)
        h = h.to(default_dtype)
        c = c.to(default_dtype)
        # print("gggghg")
        # print(D_embedding.dtype)
        # print(h.dtype)
        ...
        # print("gghghghgg")
        # print(temp.dtype)
        for sequential_block in self.output_nn:
            for layer in sequential_block:
                if isinstance(layer, nn.Linear):
                    # print(layer.weight.dtype)
                    if layer.weight.dtype == torch.float64:
        ...
        if (mask) > 0:
            ...
            if sum(heating) > 0:
                ...
                # print(mask.shape)
                #  print(heating.shape)
                # print(E[mask & heating, :])
                # print(E[mask & heating, :].clone().shape)
                #  print(self.a(power[mask & heating]).shape)
                #   print(self.a_scaling) #that is true
