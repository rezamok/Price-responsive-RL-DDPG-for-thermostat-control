import pandas as pd
from models.util import standardize, inverse_standardize, normalize, inverse_normalize
...
def prepare_data(data: pd.DataFrame, interval: int, model_kwargs: dict, Y_columns: list, X_columns: list = None, verbose: int = 2):
    ...
    return dataset
def prepare_data_nestli(data: pd.DataFrame, interval: int, model_kwargs: dict, Y_columns: list, X_columns: list = None, verbose: int = 2):
    """
    Pipeline of actions to take to prepare the data for some model.
