...
from models.modules_mina import PCNN, S_PCNN, M_PCNN, LSTM
from models.data import prepare_data
from models.util import model_save_name_factory, format_elapsed_time, inverse_normalize, inverse_standardize
...
class Model:
    ...
    #             load: bool = True):
        ...
        # self.valve_column = valve_column if isinstance(valve_column, list) else [valve_column]
        ...
        if self.neigh_column is None:
            print('\nSanity check of the columns:\n', [(w, [self.dataset.X_columns[i] for i in x])
                                                       for w, x in zip(['Case', 'Room temp', 'Room power', 'Out temp'],
                                                                       [[self.case_column], self.temperature_column,
                                                                        self.power_column,
                                                                        [self.out_column]])])
        else:
            print(self.dataset.X_columns)
            print('\nSanity check of the columns:\n', [(w, [self.dataset.X_columns[i] for i in x])
                                                       for w, x in zip(['Case', 'Room temp', 'Room power', 'Out temp',
                                                                        'Neigh temp'],
                                                                       [[self.case_column], self.temperature_column,
    ...
    def get_sequences(self, X: pd.DataFrame = None, Y: pd.DataFrame = None) -> list:
        ...
        if self.verbose > 0:
            print(
                f"Number of sequences for the model {self.name}: {len(heating_sequences)} heating sequences and " f"{len(cooling_sequences)} cooling sequences.")

        # Return the sequences
        return heating_sequences, cooling_sequences

    def train_test_validation_separation(self, validation_percentage: float = 0.2,
                                         test_percentage: float = 0.0) -> None:
        """
        ...
        for sequences in [self.heating_sequences, self.cooling_sequences]:
            if len(sequences) > 0:
                ...
                while True:
                    if (sequences[train_validation_sep - 1][1] < sequences[train_validation_sep][0]) | (
                            train_validation_sep == 1):
                        break
                    train_validation_sep -= 1
                if test_percentage > 0.:
                    while True:
                        if (sequences[validation_test_sep - 1][1] < sequences[validation_test_sep][0]) | (
            ...
            self.train_indices = self.train_sequences
            self.test_indices = self.test_sequences
            print("sasasas")
    ...
    def get_normalization_variables(self):
        ...
        normalization_variables['Room'] = [
            torch.Tensor(self.dataset.min_[self.temperature_column].values).to(self.device),
        ...
        if self.neigh_column is not None:
            normalization_variables['Neigh'] = [
                torch.Tensor(self.dataset.min_[self.neigh_column].values).to(self.device),
        ...
        normalization_variables['Out'] = [torch.Tensor([self.dataset.min_[self.out_column]]).to(self.device),
                                          torch.Tensor([(self.dataset.max_ - self.dataset.min_)[self.out_column]]).to(
                                              self.device)]
    ...
    def fit(self, n_epochs: int = None, print_each: int = 5) -> None:
        ...
        for epoch in range(trained_epochs, trained_epochs + n_epochs):
            ...
            for num_batch, batch_sequences in enumerate(
                    self.batch_iterator(iterator_type="validation", batch_size=2 * self.batch_size, shuffle=False)):
            ...
            if 'PiNN' in self.name:
                _validation_loss = sum([l * s for l, s in zip(_validation_losses, train_sizes)]) / sum(
                    _validation_sizes)
