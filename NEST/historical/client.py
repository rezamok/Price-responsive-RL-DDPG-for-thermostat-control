...
Small adaptations: Loris Di Natale
"""
import os
import sys
import shutil
...
if __name__ == "__main__":
    # To make the parent folder visible
...
from util.util import login_from_file

USE_CL: bool = True  #: Whether to use the command line for the login.
if not USE_CL:
    from NEST.historical.pw_gui import get_pw
else:
    from NEST.historical.pw_cl import get_pw
...
# Add depth
login_path = os.path.join(curr_dir.parent.parent.parent.parent, "rest_login.txt")

#: Where to put the local copy of the data.
save_dir = os.path.join("..", "saves", "Data") if sys.platform == "win32" else os.path.join("..", "saves", "Data")
...
def _get_data_folder(name: str, start_date: str, end_date: str, unit: str) -> str:
    """
    ...
    ### Modification Loris Di Natale
    # check path exists
    if not os.path.isdir(save_dir):
        os.mkdir(save_dir)
    ### Modification Loris Di Natale

    full_name = f"{unit}__{start_date}__{end_date}__{name}"
...
class _Client(object):
    ...
    def __init__(self, name: str, start_date: str, end_date: str, unit:str, verbose: int = 0):
    ...
    def read(self, df_data: List[str]) -> Optional[NestDataT]:
        ...
        self._auth = HttpNtlmAuth(self._DOMAIN + "\\" + username, pw)
        ...
        for ct, column in enumerate(df_data):
            ...
            if response.status_code != 200:
                print("Server didn't return an 'OK' response.  Content was: {!r}".format(response.text))
            else:
    ...
    def read_offline(self) -> NestDataT:
        ...
        # Get folder name
        data_dir = _get_data_folder(self.name, self.start_date, self.end_date, unit=self.unit)
    ...
    def write_np(self, overwrite: bool = False) -> None:
        ...
        data_dir = _get_data_folder(name, self.start_date, self.end_date, self.unit)
        ...
        assert len(self.np_data) != 0, "Nothing to save!"
...
class DataStruct:
    ...
    def __init__(self, id_list: Union[List[int], List[str]], name: str, start_date: str, end_date: str, unit: str):
        ...
        self.unit = unit
        self.REST = _Client(self._name, self._start_date, self._end_date, self.unit)
    ...
    def copy(self) -> "DataStruct":
        ...
        return DataStruct(self.data_ids.copy(), self.name, self.start_date, self.end_date)
    ...
    def _slice(self, start_ind: int, end_ind: int) -> "DataStruct":
        ...
        assert -n <= start_ind <= n and -n <= end_ind <= n, f"Indices: {start_ind} or {end_ind} out of range (n = {n})!"
        ...
        return DataStruct(new_ids, new_name, self.start_date, self.end_date)
    ...
    def __add__(self, other: "DataStruct") -> "DataStruct":
        ...
        assert self.start_date == other.start_date, "Incompatible start date!"
        assert self.end_date == other.end_date, "Incompatible start date!"
        return DataStruct(new_ids, new_name, self.start_date, self.end_date)
    ...
    def set_end(self, end_str: str = None) -> None:
        ...
        if end_str is not None:
            ...
            init_str = "0000-00-00"
            curr_str = init_str
            assert name_len > 0, "What the fuck??"
    ...
    def get_data_folder(self) -> str:
        ...
        """
        return _get_data_folder(self.name, self.start_date, self.end_date, self.unit)
    ...
    def get_data(self, verbose: int = 0) -> Optional[Tuple[List, List]]:
        ...
        assert len(ret_val) == len(meta_data) == len(self.data_ids), "Something fucked up horribly!"
...
def example():
    ...
    """
    # Example data.
    test_data = DataStruct(id_list=[421100171, 421100172], name="Test", start_date="2019-08-08", end_date="2019-08-09")
...
# Test DataStruct
TestData = DataStruct(id_list=[421100171, 421100172], name="Test", start_date="2019-08-08", end_date="2019-08-09", unit='UMAR')
